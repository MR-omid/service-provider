...
from vendor.custom_exception import InvalidResponseError as Error, TimeoutResponseError
...
class Client(SelectionMixin):
    ...
    def set_viewport_size(self, width, height):
        ...
        self.conn.issue_command("WindowResize", self.conn.issue_command("GetWindowHandle"), width, height)
    ...
    def set_attribute(self, attr, value=True):
        """ Sets a custom attribute for our Webkit instance. Possible attributes are (deprecated) :
        ...
        #                         value)
        ApiLogging.info("%s method is deprecated" % self._normalize_attr(attr))
    ...
    def block_url(self, url):
        self.conn.issue_command("BlockUrl", url)

    def allow_url(self, url):
        self.conn.issue_command("AllowUrl", url)

    def set_unknown_url_mode(self,mode):
        """ mode [block, warn] """
        self.conn.issue_command("SetUnknownUrlMode",mode)

    def set_url_black_list(self, urls):
        """ add urls to black list """
        # NOTE: call this method, clear old values every time
        self.conn.issue_command("SetUrlBlacklist", urls)

    def set_ignore_http_exception(self, value):
        """ set ignore http raise exception """
        self.conn.silent_http_exception = value

    def go_back(self):
        self.conn.issue_command('GoBack')

    def set_skip_image_loading(self,skip):
        self.conn.issue_command("SetSkipImageLoading",skip)
...
class ServerConnection(object):
    ...
    def __init__(self, server=None):
        ...
        self.server_instance = (server or get_default_server())
        self._sock = self.server_instance.connect()
        self.buf = SocketBuffer(self._sock)
        self.issue_command("IgnoreSslErrors")
        self.silent_http_exception = False
    ...
    def _read_response(self):
        ...
        if result != "ok":
            if self.silent_http_exception and 'because of error loading' in msg:
                return None
            parsed_error = json.loads(msg)
            if 'class' not in parsed_error:
                parsed_error['class'] = 'InvalidResponseError'
            if 'message' not in parsed_error:
                parsed_error['message'] = 'unknown error occurs'

            if parsed_error['class'] == 'TimeoutError':
                    raise TimeoutResponseError(parsed_error['message'])
            else:
                raise Error(parsed_error['message'])
